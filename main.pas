...
{$R *.dfm}
 ...
 ComprobvtaAdd_fm,  DashBoard, Main_Tablas,
 Vcl.Themes,
...
begin
  ...
  if EstaCreado('Archivos') then Exit;

  tabSheet := TTabSheet.Create(PageControlMain);
  tabSheet.PageControl := PageControlMain;

  FmMainTablas := TFmMainTablas.Create(tabSheet);
  FmMainTablas.Caption := 'Archivos';
  FmMainTablas.Parent := tabSheet;
  PageControlMain.ActivePage := tabSheet;
  tabSheet.Caption := FmMainTablas.Caption;
  FmMainTablas.show;
