...
{  Sender.Canvas.Brush.Style := bsClear;
  ...
  Sender.Canvas.Rectangle(10, 10, 20, 20);}
...
begin
  if not DialogoBox('¿Enviar por email al cliente ?','Envío de comprobantes', 1,1)  then
   exit;
  ...
  if Cliente.email = ''  then
   Begin
    mensaje('El cliente no tiene el campo email cargado.','Envío de comprobantes',1);
    exit;
    end;
  ...
  SendMail( Cliente.email, 'Envío de comprobante.', 'Buenos días. Adjuntamos comprobante. Saludos. ', ArchivoNombre);
  Cliente.free;
end;
