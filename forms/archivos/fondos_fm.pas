...
begin
  ...
  if EDITAR then
   begin
     ...
     FondosEditar(Fondo);
     ListaActiva.Objects[ListView1.ItemIndex] := Fondo;
     EventosAgregar('Editar rubro '+ Ed_id.text  );
   end
  else
   begin
    FondosAgregar(Fondo);
    ListaCompleta.AddObject(Fondo.Descripcion, Fondo);
    ListaActiva.AddObject(Fondo.Descripcion, Fondo);
...
begin
  ...
  frxReport1.LoadFromFile( AjusteObj.CarpReportes+'\fondos.fr3')   ;
...
begin
 ...
 Begin
  ...
  Listview1.items.BeginUpdate;
   TFondo(ListaActiva.Objects[Listview1.ItemIndex]).Descripcion  := '**** BORRADO *****';
  ListView1.Items.EndUpdate;
 End;
